...
def tan_1D_reduced_shear(theta, conc):
    ...
    #implement corrections suggested by James
    ellip_scale = 1.11
    M_200 = profiles.nfwM200(conc,z_halo)/1e14 #gives M200 in M_sun 
    del_c, r_s = profiles.nfwparam(M_200,z_halo)
    kappa = profiles.nfwkappa(del_c, r_s, x, z_halo, z_source) 
    corr = (1+0.79)*kappa/ellip_scale

    red_shear= (kappa_bar-azim_kappa)/(1-azim_kappa)*corr
