"""
This file contains a number of utility functions, most developed during my PhD.
Some borrowed from fellow graduate students
And some written after the fact.
"""
import sys
import numpy
from math import pi
...
def argnearest(array, value):
    '''
    Returns the index of the member of the 'array' that is nearest the
    specified 'value'.
    '''
    idx = (numpy.abs(array - value)).argmin()
    return idx
...
def filtercat(cat, param_index, min_param, max_param, min_inc=True,
              max_inc=True, verbose=True):
    '''
    ...
    if min_param is not None and max_param is not None:
        if min_param >= max_param:
            print('filtercat: Error, the minimum bound is greater than or',
                  'equal to the maximum bound, exiting.')
            sys.exit()
    if min_param is not None:
        if min_inc is True:
            mask_min = cat[:, param_index] >= min_param
        elif min_inc is False:
            mask_min = cat[:, param_index] > min_param
    else:
        mask_min = True
    if max_param is not None:
        ...
        elif max_inc is False:
            mask_max = cat[:, param_index] < max_param
    else:
        mask_max = True
    if min_param is not None or max_param is not None:
...
def catfilter(min_param, max_param, cat, param_index, min_inc=True,
              max_inc=True, verbose=True):
    '''
    Filters the input catalog (cat) based on some minumum (min_param) and
    maximum (max_param) cluster pair parameter with column number (param_index)
...
def angdist(ra1, dec1, ra2, dec2):
    ...
    if ra1 < ra2:
        ra1, ra2 = ra2, ra1
    cosdist = (cos((ra1 - ra2) * d2r) * cos(dec1 * d2r) * cos(dec2 * d2r) +
...
def angcomp(ra1, dec1, ra2, dec2, method=3):
    ...
    angular_distance function above. Distance is returned as degrees, and
    method chooses a more or less accurate way of determining the distance
    (with 1 being the fastest/least accurate).
    from astorlib.py
    ...
    if method == 1:
        ...
        div = 1.
        if method == 3:
            div = (sin(dec2) * sin(dec1) +
                   cos(dec2) * cos(dec1) * cos((ra1 - ra2)))
        deltara = cos(dec2) * sin(ra1 - ra2) / div
        deltadec = - (sin(dec2) * cos(dec1) -
                      cos(dec2) * sin(dec1) * cos(ra1 - ra2) / div)
...
def angendpt(ra1, dec1, d_arcmin, pa):
    ...
    x = (sin(dec2 * d2r) * sin(dec1 * d2r) +
         cos(dec2 * d2r) * cos(dec1 * d2r) * cos((delta_ra_eq * d2r)))
...
def readheader(catalog):
    ...
    if header['column'][0] == 1:
        ...
        print('readheader: the catalog is not ttype indexed, please index',
              'using format ttype(column#)=(column name), exiting')
        sys.exit()
...
def distancematch(objid_1, ra_1, dec_1, objid_2, ra_2, dec_2, outputfile,
                  tolerance=2, stampsize=10):
    '''
    ...
    fh.write('#This catalog was created by the distancematch function of ',
             'tools.py and matches objects in catalog 2 \n')
    fh.write('#with objects in catalog 1 based on their angular separation ',
             '(matchdelta; units=arcsec) and user input.\n')
    ...
    for i in numpy.arange(N_2):
        ...
        ramin = (ra_2[i] - stampsize /
                 (60.**2 * 2. * numpy.cos(dec_2[i] * numpy.pi / 180.)))
        ramax = (ra_2[i] + stampsize /
                 (60.**2 * 2. * numpy.cos(dec_2[i] * numpy.pi / 180.)))
        ...
        delta = numpy.zeros(N)
        for k in range(N):
            delta[k] = numpy.abs(angdist(ra_flt[k], dec_flt[k], ra_2[i],
                                 dec_2[i]) * 60**2)
        ...
        if j == 1:
            ...
            if numpy.size(delta) != 0:
                # then present the user with the nearest objects within the
                # stamp
                ...
                selection = input('Enter the number of the correct object match: ')
            ...
            selection = input('Enter the number of the correct match: ')
            if numpy.size(numpy.arange(k + 1) == int(selection)) == 0:
                selection = rawinput("Input invalid. Please enter a valid number.: ")
            if selection == str(j):
...
def dec2deg(dec):
    ...
    if comp[0][0] == "-":
            ...
            sign = 1.
    return sign * (float(comp[0]) + float(comp[1]) / 60. +
                   float(comp[2]) / 3600.)
...
def addwcs(fitsfile, option, parentfits=None, ra_bounds=None, dec_bounds=None,
    ...
    naxis2 = hdr['naxis2']
    if option == 'parent':
        if parentfits is None:
            print("addwcs: error, parentfits must be specified if ",
                  "option=='parent', exiting")
            sys.exit()
        ...
        sys.exit()
    hdr.add_history("This wcs was created using Will Dawson's python program ",
                    "tools.addwcs")
