...
def load_pickles_to_df(par, prefix, index, msun1e14 = True,
    ...
    for i in range(len(par)):
        if(verbose): print 'loading '+ par[i]
    ...
    if len(radiomask) != len(dataarray):
        ...
        for n in range(len(temp)):
            if temp[n] != 0.0 :
                ...
                ncounter+=1
        #print 'number of non-zero entries for data array after masking is', ncounter
...
def percentdiff(x,prefix,prob=None,N_bins=100,histrange=None,x_lim=None,
	...
	#print i,"th iteration"
    	...
    	hist, binedges, tmp = pylab.hist(x[:partsize],bins=N_bins,histtype='step',                              weights=prob[:partsize],range=histrange,color='k',
                                linewidth=2)
    ...
    for n in range(1,nparts-1):
        x_perdiff[n-1]= (data[nparts-2]-data[n-1])*2*100/(data[nparts-2]+data[n-1])
...
def histplot1d_pdf(x,prefix,prob=None, N_bins=100,histrange=None,x_lim=None,y_lim=None,x_label=None,y_label=None,legend=None):
    ...
    pdf, bins, holder = pylab.hist(x,bins=N_bins,histtype='step',weights=prob,range=histrange,color='k',linewidth=2,normed=1)


   #print prefix+' hist array with len= ',len(hist),' is '
    #print hist
    #print prefix+' binedges is with len= ',len(binedges),' is '
    #print binedges
    #print prefix+' prob is with len= ',len(prob),' is '
    #print prob
...
def histplot1d(x,prefix,prob=None,norm=False, N_bins=100,histrange=None,x_lim=None,y_lim=None,x_label=None,y_label=None,legend=None):
    fig = pylab.figure()
    hist, binedges, tmp = pylab.hist(x,bins=N_bins,histtype='step',weights=prob,range=histrange,color='k',linewidth=2,normed=norm)
...
## Plot the percentage differences between different % of data within Will's   2 million iterations
def plot_perdiff(perdiff, labels, title):
    fig = plt.figure()
    for i in range(5):
		plt.plot(range(1,len(perdiff[:,1])+1),perdiff[:,i], label=labels[i])
    ax = plt.subplot(111)
    box = ax.get_position()
...
def prior_diff(data1,data2,prefix,prob1=None,prob2=None,N_bins=100,
               histrange=None,x_lim=None,y_lim=None,x_label=None,y_label=None,legend=None):

    fig = pylab.figure()

    #bin data 1 first
    hist2, binedges2, tmp2 = pylab.hist(data2,bins=N_bins,histtype='step',weights=prob2,range=histrange,color='#ff0000',linewidth=2)
    ...
    # Create location and confidence interval line plots
    pylab.plot((loc1,loc1),(pylab.ylim()[0],pylab.ylim()[1]),'--',linewidth=2,color='#6495ed',label='$C_{BI}$')
    pylab.plot((ll_68_1,ll_68_1),(pylab.ylim()[0],pylab.ylim()[1]),'-.',linewidth=2,color='#7fffd4',label='68% $IC_{B_{BI}}$')
    pylab.plot((ul_68_1,ul_68_1),(pylab.ylim()[0],pylab.ylim()[1]),'-.',linewidth=2,color='#7fffd4')
    pylab.plot((ll_95_1,ll_95_1),(pylab.ylim()[0],pylab.ylim()[1]),':',linewidth=2,color='#87ceeb',label='95% $IC_{B_{BI}}$')
    pylab.plot((ul_95_1,ul_95_1),(pylab.ylim()[0],pylab.ylim()[1]),':',linewidth=2,color='#87ceeb')
    ...
    # Create location and confidence interval line plots
    pylab.plot((loc2,loc2),(pylab.ylim()[0],pylab.ylim()[1]),'--',linewidth=2,color='#ff4500',label='$C_{BI}$')
    pylab.plot((ll_68_2,ll_68_2),(pylab.ylim()[0],pylab.ylim()[1]),'-.',linewidth=2,color='#ff8c00',label='68% $IC_{B_{BI}}$')
    pylab.plot((ul_68_2,ul_68_2),(pylab.ylim()[0],pylab.ylim()[1]),'-.',linewidth=2,color='#ff8c00')
    pylab.plot((ll_95_2,ll_95_2),(pylab.ylim()[0],pylab.ylim()[1]),':',linewidth=2,color='#ffa500',label='95% $IC_{B_{BI}}$')
    pylab.plot((ul_95_2,ul_95_2),(pylab.ylim()[0],pylab.ylim()[1]),':',linewidth=2,color='#ffa500')
...
def histplot2d_2contour(x1,y1,x,y,prefix,prob1=None,prob=None,N_bins=100,
    ...
    fig = pylab.figure()
    # Countours
    CS = pylab.contour(X1,Y1,H1,(h_2sigma,h_1sigma),linewidths=(2,2),colors=('#a4a4a4','#6e6e6e'))
    # imshow
